...
def youtube_login(email, password):
    ...
    ch.add_argument("--disable-infobars")
    ch.add_argument("--log-level=3")
    ch.add_argument("--disable-extensions")
    driver = webdriver.Chrome(options=ch, executable_path=CM().install())
...
def comment_page(driver, urls, comment):
    ...
    if check_exists_by_xpath(driver, '//*[@id="shorts"]/ytd-message-renderer'):
        print("skiped")
        return comment_page(driver, urls, random_comment())
    ...
    #After commenting like button click-

    driver.find_element_by_css_selector('#like-button.ytd-comment-action-buttons-renderer').click()
    print('Liked')
    stop(5)
    return comment_page(driver, urls, random_comment())
...
#-----Note It will run continue, To stop the bot close opned chrome.------- 
